...
export default function CostExplorer() {
  ...
  const handleChartData = (data) => {
    ...
    const months = [
      ...new Set(data.map((item) => item["USAGE_MONTH"] || item.value)),
    ].sort();
    ...
    const formattedTableData = Object.entries(serviceData).map(
      ([serviceName, amounts]) => {
            ...
            <div className="fusionchart_container">
              <div
                className={
                  showFilterSidebar
                    ? "fusionchart_wrapper"
                    : "fusionchart_wrapper_without_filter"
                }>
                ...
                <div className="additonal_content1">
                  <pre>We are showing up to top 1000 results</pre>
                </div>
                <div className="table_section">
                  <div className="table_header">
                    <h3>Service Usage Table</h3>
                      ...
                      <thead>
                        <tr>
                          ...
                          {[
                            ...new Set(
                              chartData.map(
                                (item) => item["USAGE_MONTH"] || item.value
                              )
                            ),
                          ]
                      ...
                      <tbody>
                        {Object.entries(
                          chartData.reduce((acc, item) => {
                            ...
                            const amount =
                              item["TOTAL_AMOUNT"] || item.value || 0;
                            acc[serviceName].push(amount);
